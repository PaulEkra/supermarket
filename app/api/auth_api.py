...
from users.schemas.token.refresh_token_schema_response import RefreshTokenResponseSchema
from users.schemas.user.user_login_schema import UserLoginSchema
...
from users.schemas.token.refresh_token_schema import RefreshTokenSchema
...
def login_for_access_token(form_data: UserLoginSchema, session: Session = Depends(get_session)):
    ...
    access_token, expires_at = create_access_token(data={"username": db_user.username})
    refresh_token = create_refresh_token(data={"username": db_user.username})
    return AccessTokenSchema(access_token=access_token, refresh_token=refresh_token, token_type="bearer", expires_at=expires_at)
...
def refresh_access_token(refresh_token: RefreshTokenSchema, session: Session = Depends(get_session)):
    ...
    access_token, expires_at = create_access_token(data={"username": db_user.username})
    return {"access_token": access_token, "token_type": "bearer", "expires_at": expires_at, "refresh_token": refresh_token.refresh_token}